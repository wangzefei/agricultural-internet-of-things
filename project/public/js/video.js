...
function setCameraList() {
    ...
    for (var i = 0; i < cameraList.length; i++) {
        ...
        var li = $('<li onclick="cameraClick(this);">').data("camera", camera);
        li.append('<a href="javascript:void(0)" title="' + camera.name + '">' + camera.name +
        '<i id="' + camera.id + '_' + camera.port + '" class="wait" title="' + camera.online + '"></i></a>');
